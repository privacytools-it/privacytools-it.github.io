...
navSections.forEach(navSection => { navSection.addEventListener("toggle", navSectionsToggle); });
...
function navSectionsClose(event) {
	...
	if (event.target.matches(".nav-summary") || event.target.parentNode.matches(".nav-summary")) return;
	navSections.forEach(navSection => {
